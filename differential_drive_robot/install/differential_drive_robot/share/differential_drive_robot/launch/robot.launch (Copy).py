...
def generate_launch_description():
    ...
    gz_worlds_path = os.path.join(get_package_share_directory(
        robot_pkg_name), 'worlds', 'chats.world')

    print(gz_worlds_path)
    ...
    gazebo_launch = IncludeLaunchDescription(
        gazebo_pkg_launch,
        launch_arguments={
            'gz_args': '-r -v v4 empty.sdf',   # Run with verbose output and an empty world
##            'gz_args': 'empty.sdf',   # Run with verbose output and an empty world
#            'gz_args': ['-r -v v4 ', gz_worlds_path],   # Run with verbose output and an empty world
#            'gz_args': f"-r -v 4 {gz_worlds_path}",   # Run with verbose output and an empty world
#            'gz_args': '-r -v v4 /home/jlegarrec/gazebo_models_worlds_collection/worlds/yosemite.world',   # Run with verbose output and an empty world
